...
def gen_palette [palette: record] {
    return {
...
export def apply_palette [flavor?: string] {
    if $flavor == "latte" {
        let palette = gen_palette $latte
        return $palette
    }

    if $flavor == "macchiato" {
        let palette = gen_palette $macchiato
        return $palette
    }

    if $flavor == "mocha" {
        let palette = gen_palette $mocha
        return $palette
    }

    let palette = gen_palette $frappe
    return $palette
}
